...
def get_production_history_for_order(order_id: str) -> QuerySet:
    ...
    return ProductionHistory.objects.filter(order_id=order_id).select_related("changed_by").order_by("changed_at")


def get_events_for_order(order_id: str) -> QuerySet:
    """Retorna todos los eventos de un pedido en orden cronológico."""
    return OrderEvent.objects.filter(order_id=order_id).select_related("created_by").order_by("created_at")
...
def get_event_by_id(event_id: str, order_id: str) -> OrderEvent | None:
    ...
    return OrderEvent.objects.filter(id=event_id, order_id=order_id).select_related("created_by").first()
