...
class QuoteSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            "id", "order_id",
            "weight_grams", "print_time_hours",
            "material_cost", "energy_cost", "labor_cost",
            "post_processing_cost", "packaging_cost", "risk_cost", "shipping_cost",
            "subtotal", "profit_amount", "discount_amount", "total_price",
            "quote_status", "accepted_at", "rejected_at", "expires_at",
...
class CreateQuoteSerializer(serializers.Serializer):
    ...
    print_time_hours = serializers.DecimalField(
        max_digits=10, decimal_places=2, min_value=Decimal("0.01")
    )
...
class CalculateSerializer(serializers.Serializer):
    ...
    priority = serializers.ChoiceField(
        choices=[("NORMAL", "Normal"), ("URGENT", "Urgent"), ("EXPRESS", "Express")]
    )
    shipping_cost = serializers.DecimalField(
        max_digits=10, decimal_places=2,
        min_value=Decimal("0.00"),
